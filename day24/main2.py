...
if __name__ == '__main__':
    ...
    while (o := len(path_options)) > 0:
        ...
        if iteration % 100_000 == 0:
            logging.debug(f"{iteration}: m{minute}, p{position}, o{o:,}, f{fewest_minutes}")
    ...
    start_position, destination = destination, start_position
    positions_by_minute = defaultdict(set)
    positions_by_minute[fewest_minutes].add(start_position)
    path_options = deque([(fewest_minutes, start_position)])
    iteration = 0
    fewest_minutes = None
    while (o := len(path_options)) > 0:
        (minute, position) = path_options.popleft()
